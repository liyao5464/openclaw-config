...
def create_placeholder_image():
    """创建一个 200x200 的白色 PNG"""
    from PIL import Image
    img = Image.new('RGB', (200, 200), color='white')
    thumb_path = "/tmp/thumb_200.png"
...
def main():
    ...
    print(f"\n🖼️ 创建并上传封面图...")
    try:
        thumb_path = create_placeholder_image()
        print(f"✅ 封面图创建成功: {thumb_path}")
    except Exception as e:
        print(f"❌ 创建封面图失败: {e}")
        print("尝试使用 curl 下载示例图片...")
        # 如果 PIL 不可用，下载一个示例图片
        os.system("curl -sL 'https://picsum.photos/200/200' -o /tmp/thumb_200.png")
        thumb_path = "/tmp/thumb_200.png"
